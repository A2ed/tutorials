...
def model_eval(fitted_model, data, cv=False):
    ...
    precision, recall, pr_thresholds = metrics.precision_recall_curve(y_test, fitted_model.predict(X_test))
    # calculate average precision
    average_precision = metrics.average_precision_score(y_test, fitted_model.predict(X_test))
    ...
    # display results
    print(f'Accuracy on the training data is {round(metrics.accuracy_score(y_train, fitted_model.predict(X_train)), 3)}.')

    # display model accuracy
    print(f'Accuracy on the test data is {round(metrics.accuracy_score(y_test, fitted_model.predict(X_test)), 3)}.')

    # display AUC score
    print(f'The model AUC for ROC curve of the test data is {round(metrics.roc_auc_score(y_test, y_pred_prob), 3)}')
...
def model(classifier, data, cv=False):
    ...
    # evaluate model
    model_eval(fitted_model=clf, data=data, cv=cv)
    # return fitted model
    return clf